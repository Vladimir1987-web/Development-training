        ...
        if user_sort_rub == "ДА":
            if answer_user_file == 1:
                filtered_transactions = [
                    x for x in list_sort_date if x["operationAmount"]["currency"]["code"] == "RUB"
                ]
            elif answer_user_file == 2 or answer_user_file == 3:
                filtered_transactions = [x for x in list_sort_date if x["currency_code"] == "RUB"]
...
while user_sort_description not in user_answer_description:
    ...
    if user_sort_description not in user_answer_description:
        ...
        if user_sort_description == "ДА":
            ...
            filter_trans = process_bank_search(filtered_transactions, word)
